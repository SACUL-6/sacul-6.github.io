...
for (var x = 0; x < 4; x++) {
  ...
  for (var y = 0; y < 4; y++) Grid.prototype.indexes[x].push({ x: x, y: y });
}
(Grid.prototype.build = function () {
  for (var e = 0; e < this.size; e++)
    for (var t = (this.cells[e] = []), i = 0; i < this.size; i++) t.push(null);
}),
  ...
  (Grid.prototype.clone = function () {
    (newGrid = new Grid(this.size)), (newGrid.playerTurn = this.playerTurn);
    for (var e = 0; e < this.size; e++)
      for (var t = 0; t < this.size; t++)
    ...
    return (
      ...
      o.x.forEach(function (e) {
        o.y.forEach(function (o) {
          if (((t = r.indexes[e][o]), (i = r.cellContent(t)))) {
            ...
            if (c && c.value === i.value && !c.mergedFrom) {
              ...
              (u.mergedFrom = [i, c]),
                ...
                (n += u.value),
                2048 === u.value && (a = !0);
            } else r.moveTile(i, h.farthest);
            r.positionsEqual(t, i) || ((r.playerTurn = !1), (s = !0));
          }
        });
      }),
      { moved: s, score: n, won: a }
    );
  }),
  (Grid.prototype.computerMove = function () {
    this.addRandomTile(), (this.playerTurn = !0);
  }),
  (Grid.prototype.buildTraversals = function (e) {
    for (var t = { x: [], y: [] }, i = 0; i < this.size; i++)
      t.x.push(i), t.y.push(i);
    return (
      1 === e.x && (t.x = t.x.reverse()), 1 === e.y && (t.y = t.y.reverse()), t
    );
  }),
  ...
  (Grid.prototype.islands = function () {
      ...
      var e = this,
        t = function (i, r, l) {
          if (
            ...
            for (direction in ((e.cells[i][r].marked = !0), [0, 1, 2, 3])) {
    ...
    for (r = 0; r < 4; r++)
      for (l = 0; l < 4; l++)
        this.cells[r][l] &&
          !this.cells[r][l].marked &&
          (i++, t(r, l, this.cells[r][l].value));
    return i;
  }),
    ...
    for (var e = 0, t = 0; t < 4; t++)
      for (var i = 0; i < 4; i++)
        if (this.cellOccupied(this.indexes[t][i]))
          for (
            var r =
                Math.log(this.cellContent(this.indexes[t][i]).value) /
                Math.log(2),
              l = 1;
            l <= 2;
            l++
  ...
  (Grid.prototype.monotonicity = function () {
    for (
      var e = this, t = [], i = [], r = 0, l = { x: 0, y: 0 }, o = 0;
      o < 4;
      o++
    ) {
      t.push([]), i.push([]);
      for (var s = 0; s < 4; s++)
        t[o].push(!1),
          i[o].push(!1),
          this.cells[o][s] &&
            this.cells[o][s].value > r &&
            ((r = this.cells[o][s].value), (l.x = o), (l.y = s));
    }
    (increases = 0),
      (cellQueue = [l]),
      (i[l.x][l.y] = !0),
    ...
    for (
      var n = function (r) {
        ...
        (l = e.cellOccupied(r)
          ? Math.log(e.cellContent(r).value) / Math.log(2)
          : 0),
        [0, 1, 2, 3])) {
          var o = e.getVector(direction),
          ...
          e.withinBounds(s) &&
            !t[s.x][s.y] &&
            (e.cellOccupied(s) &&
              ((targetValue = Math.log(e.cellContent(s).value) / Math.log(2)),
              targetValue > l && (increases += targetValue - l)),
            i[s.x][s.y] || (cellQueue.push(s), (i[s.x][s.y] = !0)));
        }
        if (0 == markAfter) {
      ...
      markAfter--, n(cellQueue.shift());
    return -increases;
  }),
  (Grid.prototype.monotonicity2 = function () {
    for (var e = [0, 0, 0, 0], t = 0; t < 4; t++)
      for (var i = 1 + (l = 0); i < 4; ) {
        for (; i < 4 && !this.cellOccupied(this.indexes[t][i]); ) i++;
        i >= 4 && i--,
          (o = this.cellOccupied({ x: t, y: l })
            ? Math.log(this.cellContent(this.indexes[t][l]).value) / Math.log(2)
            : 0) >
          (s = this.cellOccupied({ x: t, y: i })
            ? Math.log(this.cellContent(this.indexes[t][i]).value) / Math.log(2)
            : 0)
            ? (e[0] += s - o)
            : s > o && (e[1] += o - s),
          (l = i),
    ...
    for (var r = 0; r < 4; r++) {
      var l;
      for (i = 1 + (l = 0); i < 4; ) {
        for (; i < 4 && !this.cellOccupied(this.indexes[i][r]); ) i++;
        var o, s;
        i >= 4 && i--,
          (o = this.cellOccupied({ x: l, y: r })
            ? Math.log(this.cellContent(this.indexes[l][r]).value) / Math.log(2)
            : 0) >
          (s = this.cellOccupied({ x: i, y: r })
            ? Math.log(this.cellContent(this.indexes[i][r]).value) / Math.log(2)
            : 0)
            ? (e[2] += s - o)
            : s > o && (e[3] += o - s),
          (l = i),
  ...
  (Grid.prototype.isWin = function () {
    for (var e = 0; e < 4; e++)
      for (var t = 0; t < 4; t++)
        if (
          this.cellOccupied(this.indexes[e][t]) &&
          2048 == this.cellContent(this.indexes[e][t]).value
        )
          return !0;
    return !1;
  });
