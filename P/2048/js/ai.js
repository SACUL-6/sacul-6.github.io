...
(AI.prototype.eval = function () {
  ...
  return (
    0.1 * this.grid.smoothness() +
    1 * this.grid.monotonicity2() +
    2.7 * Math.log(e) +
    1 * this.grid.maxValue()
  );
}),
    ...
    if (this.grid.playerTurn) {
      for (var l in ((s = i), [0, 1, 2, 3]))
        if ((I = this.grid.clone()).move(l).moved) {
          if ((t++, I.isWin()))
            return { move: l, score: 1e4, positions: t, cutoffs: o };
          var h = new AI(I);
          if (
            (0 == e
              ...
              : ((n = h.search(e - 1, s, r, t, o)).score > 9900 && n.score--,
                (t = n.positions),
                (o = n.cutoffs)),
            n.score > s && ((s = n.score), (a = l)),
            s > r)
          )
            return { move: a, score: r, positions: t, cutoffs: ++o };
        }
    } else {
      s = r;
      var u = [],
      ...
      for (var p in f)
        for (v = 0; v < f[p].length; v++)
          f[p][v] == d && u.push({ position: c[v], value: parseInt(p, 10) });
